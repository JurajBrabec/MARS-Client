class Field {
      ...
      if (!this.name) {
        ...
        if (["string", "number", "float", "date"].includes(value)) {
          ...
          if (value instanceof RegExp) {
            ...
            if (/\(\\w\+\)/.test(string)) this.type = "string";
  ...
  isNumber = (value) => Number(value) === value && value % 1 === 0;
...
class Table {
  ...
  set(row, fieldName, value) {
    ...
    try {
      ...
      if (result) row[this.name][fieldName] = result;
    } catch (err) {
      throw new Error(`Table "${this.name}": ${err.message}`);
    }
    return result;
  ...
  assign(row, array = [], start = 0) {
    ...
    this.fields
      .filter(
        (field, index) => !field.value && !field.regExp && index < array.length
      )
      .forEach((field) => {
        this.set(row, field.name, array[index]);
        index++;
      });
    return index;
  }
