...
const { NetBackupFile } = require("./NetBackup");
const { Convert } = require("../Convert");

class VaultXmlConvert extends Convert {
  convert(text) {
    let result = { row: [] };
          ...
          if (IPF.Enabled === "YES") {
            ...
            if (clientFilter.InclExclOpt !== "INCLUDE_ALL") {
              ...
              clients.forEach((item) => {
                const vault_item_xml = JSON.parse(
                  JSON.stringify(row.vault_item_xml)
                );
...
class VaultXml extends NetBackupFile {
  constructor(netBackup) {
    ...
    const convert = new VaultXmlConvert({ tables });
    super(netBackup, {
      file: "db/vault/vault.xml",
      transform: { delimiter: /^(?<=\<\?xml )/m, expect: /^\<\?xml /, convert },
    });
  }
}
