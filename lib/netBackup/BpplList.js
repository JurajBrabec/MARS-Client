...
class BpplListLabeledConvert extends NetBackupLabeledConvert {
  subConvert(name, text, values = {}) {
    ...
    return new NetBackupSeparatedConvert({ separator: / /, tables }).convert(
      text
    );
  }
  convert(text) {
    ...
    row = this.subConvert(table, [src.class, src.info].join(" ")).row[0];
    ...
    const values = { policyName: row[table].name };
    if (src.clients) {
      ...
      src.clients
        ...
        .filter((item) => item !== "")
        .forEach((item) => {
          const row = this.subConvert(table, item, values).row[0];
          if (this.validate(row)) result.row.push(row);
        });
    }
    if (src.schedules) {
      ...
      src.schedules
        ...
        .forEach((item) => {
          ...
          const row = this.subConvert(table, items.shift(), values).row[0];
          items.forEach((item) => {
