...
class Nbu {
  ...
  async files({ backupId, client, all }) {
    ...
    if (all) args.push("-client", await this._allClients());
    return this._readableProcess(Bpflist.Files);
  }
  async images({ days, client, all }) {
    const Bpimmedia = require("./Bpimmedia")({ nbu, target: Tables });
    const args = Bpimmedia.Images.binary.args;
    if (days) args.push("-d", ...this.dateDiff(-days).split(" "));
    if (client) args.push("-client", client);
    if (all) args.push("-client", await this._allClients());
    return this._readableProcess(Bpimmedia.Images);
  }
  clients({ status }) {
  ...
  async _allClients() {
    const batch = await this.clients({ status: false });
    const clients = batch.reduce((rows, insert) => {
      rows = rows.concat(insert.bpplclients.rows.map((row) => row.name));
      return rows;
    }, []);
    return clients;
  }
