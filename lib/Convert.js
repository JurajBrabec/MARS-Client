...
class SeparatedConvert extends Convert {
  ...
  parse(text) {
    ...
    const escape = new RegExp(`\\${source}`, "m").test(text);
...
class MultiPartSeparatedConvert extends SeparatedConvert {
  ...
  parseOthers(line) {
    return super.parse([this.first.line, line].join(this.separator.source));
  }
  parse(text) {
    let rows = [];
    text
      ...
      .forEach((line) => {
        if (this.first) {
          rows.push(this.parseOthers(line));
        } else {
          this.first = { line, row: this.parseFirst(rows, line) };
        }
      });
    return rows;
  }
