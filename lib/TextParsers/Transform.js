...
class Text {
  constructor(prop) {
    ...
    this.quoteChar = `"`;
    this.separator = ",";
    if (typeof prop === "function") this._transform = prop.bind(this);
  ...
  transform = (text) => this.set(text)._transform();
...
class Transform extends stream.Transform {
  ...
  _transform(chunk, encoding, callback) {
    this.push(this.tText().set(chunk).get());
    callback();
  }
  _flush(callback) {
    callback();
...
module.exports = { Text, Function, Transform };
