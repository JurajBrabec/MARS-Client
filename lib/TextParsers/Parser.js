...
class Transform extends stream.Transform {
  constructor(options) {
    ...
    status.source = this.constructor.name;
    const dbg = debug(this.constructor.name);
    options = {
      ...{ parser: null },
      ...options,
  ...
  pushRows(rows) {
    this.status.outObjects += rows.length;
    rows.map((row) => this.push(row));
  }
  _transform(chunk, encoding, callback) {
    if (chunk) {
      this.status.inBytes += chunk.length;
      this.status.inLines += chunk.split("\n").length;
    }
...
class BatchInsert extends Transform {
  ...
  pushRows(rows) {
    ...
    Object.keys(batch).map((table) => {
      this.status.outObjects += batch[table].rows.length;
...
class BufferedBatchInsert extends Transform {
  constructor(options) {
    ...
    super(options);
    this.status.outBuffers = 0;
  }
  pushBuffer(table) {
    this.status.outBuffers++;
...
module.exports = { Parser, Transform, BatchInsert, BufferedBatchInsert };
