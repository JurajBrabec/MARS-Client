...
class Parser {
  constructor(arg, props) {
    ...
    this.textBuffer = "";
    if (typeof arg === "function") this._parse = arg.bind(this);
    if (typeof arg === "string") this.set(arg);
  ...
  //BUFFER(string) - TODO: finish docu
  buffer = (text) => {
    this.dbg("buffering");
  ...
  //FLUSH() - flush buffer TODO: finish docu
  flush = () => {
    this.dbg("flushing");
    if (!this.isBuffering) return this;
    return this.textBuffer ? this.parse(this.textBuffer) : false;
  };
...
class Transform extends stream.Transform {
  constructor(options) {
    ...
    super(options);
    this.status = { bytes: 0, lines: 0, objects: 0 };
    this.dbg = debug(this.constructor.name);
    this.parser = options.parser;
  }
  pushRows(rows) {
    this.status.objects += rows.length || 0;
    if (rows) rows.map((row) => this.push(row));
  }
  _transform(chunk, encoding, callback) {
    this.status.bytes += chunk.length || 0;
    this.status.lines += chunk.split("\n").length || 0;
    this.pushRows(this.parser.buffer(chunk));
    callback();
  }
  _flush(callback) {
    this.pushRows(this.parser.flush());
