...
class Converter {
  constructor(params) {
    this.params = params;
    this.dbg = require("debug")("converter");
  }
  convert(text) {
    this.dbg("convert");
    let result = { row: [] };
  ...
  validateValue(fieldType, value) {
    ...
    switch (fieldType) {
      case "number":
        result = value === null ? null : parseInt(value);
        break;
      case "float":
        result = value === null ? null : parseFloat(value);
        break;
      default:
        result = value;
        break;
...
class LabeledConverter extends Converter {
  ...
  validateValue(fieldType, value) {
    return super.validateValue(fieldType, value === "" ? null : value);
  }
}
    ...
    this.params.dataDefinition.tables.forEach((table) => {
      table.fields.forEach((field) => {
        ...
        if (field.fixedValue !== undefined) {
          ...
          value = escape
            ? match[index].replace(placeHolder, `\\${separator.source}`).trim()
            : match[index].trim();
          index++;
        }
        row[table.tableName][field.fieldName] = this.validateValue(
    ...
    text
      ...
      .forEach((line, index) => {
        ...
        if (index == 0) {
          this.params.dataDefinition.tables.forEach((table) => {
            table.fields.forEach((field) => {
              let value = row[table.tableName][field.fieldName];
              if (value !== undefined) field.fixedValue = value;
              return field;
            });
          });
        } else {
